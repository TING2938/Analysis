...
#define logi(a) fprintf(outf, "(%4d)%8s: %8d\n", __LINE__, #a, a)
#define logd(a) fprintf(outf, "(%4d)%8s: %8f\n", __LINE__, #a, a)
...
logi(kzmax);
  ...
  fprintf(outf, "(%4d)before: kxmax=%d,kymax=%d,kzmax=%d\n", __LINE__, kxmax,kymax,kzmax);
  FILE *getKmax;
  static int bFirst=1, b3dOr3dc=0;
  static int kxmaxtemp, kymaxtemp, kzmaxtemp, kmaxtemp;
  char   gettemp[36];
  if(bFirst)
  {
		if((getKmax = fopen("getKmax.dat","r")) == NULL)
		{
			printf("\nNOTE: Kmax Parameter file cannot be loaded, mission terminated.\n");
			exit(0);
		}	
		fscanf(getKmax,"%s%d \n",gettemp,&kxmaxtemp);
		fscanf(getKmax,"%s%d \n",gettemp,&kymaxtemp);
...
logi(kmax);
  ...
  if(b3dOr3dc==0)
  {
	  ...
	  ThreedOrThreedc = 0;
	  /* bisheng */
	  fprintf(outf, "(%4d)xprd=%f,yprd=%f,zprd=%f,zprd_slab=%f,volume=%f\n",__LINE__, xprd,yprd,zprd,zprd_slab,volume);	  
  }
  else
  {
	  volume = xprd * yprd * zprd_slab;
  ...
  fprintf(outf, "(%4d)after: kxmax=%d,kymax=%d,kzmax=%d\n", __LINE__, kxmax,kymax,kzmax);
...
void FixConp::b_cal()
{
  ...
  for (k = 0; k < kcount; k++) {
    ...
    for (i = 0; i < nlocal; i++) {
      if (electrode_check(i)) {
	    ...
	    //printf("check =%d, k=%d,i=%d,exprl=%f,expim=%f\n",atom->molecule[i],k,i,exprl,expim);
        /* bisheng */
        //bbb[j] -= 2.0*ug[k]*(exprl*sfacrl_all[k]+expim*sfacim_all[k]); // it seems that it should be 1
	    j++;
      }
    }
  }
  ...
  //elenum_list and displs f r gathering ele tag list and bbb
  ...
  for (i = 0; i < nlocal; ++i) {
    ...
    if (electrode_check(i)) {
      ...
      for (j = 0; j < elenum_all; ++j) {
        ...
        for (k = 0; k < kcount; ++k) {
          aaa[idx1d] += 0; // 2.0*ug[k]*(csk[k][elealli]*csk[k][j]+snk[k][elealli]*snk[k][j]);
        }
		if(ThreedOrThreedc)
		{
		  //printf("ThreedOrThreedc=%d\n",ThreedOrThreedc);
		  aaa[idx1d] += 0; // CON_4PIoverV*zi*eleallx[j][2];	//if system is not slab, no need to add slab correction.	
		}
	/*bisheng*/
	//printf("zi=%f,elex=%f,V=%.10f,3dc=%f,aaa=%f\n",zi,eleallx[j][2],CON_4PIoverV,CON_4PIoverV*zi*eleallx[j][2],aaa[idx1d]);
      }
      idx1d = elei*elenum_all+elealli;
      aaa[idx1d] += 0; // CON_s2overPIS*eta-CON_2overPIS*g_ewald; //gaussian self correction
      //printf("%d %d %f %f %f\n",elei,i,x[i][0],x[i][1],x[i][2]);
    }
  }
...
void FixConp::update_charge()
{
  ...
  for (i = 0; i < nall; ++i) {
	...
	//printf("%d\n",elealli);
	  ...
	  //printf("%d %d %f %f\n",elealli,idx1d,aaa_all[idx1d],bbb_all[j]);
        }
        q[i] = eleallq_i;
	  //printf("%d %d %f %f %f %f\n",i,elealli,x[i][0],x[i][1],x[i][2],q[i]);
      } 
    }
  }
}
