...
void getMolInfo_diffType(t_topology* top, int* ngx, int** index, int grp, int** atomNum, int* nmols, double ***deloc, double **totNCM, int ncm);
...
#define NFILE asize(fnm)
    ...
    */
	...
	double **deloc, *totNCM;	
    getMolInfo_diffType(top, ngx, index, 0, &eachAtom, &nmols, &deloc, &totNCM, 2);
...
void getMolInfo_diffType(t_topology* top, int* ngx, int** index, int grp, int** atomNum, int* nmols,
    double*** deloc, double** totNCM, int ncm)
{
    ...
    *deloc = (double**)malloc(sizeof(double*) * (*nmols));
    ...
    for (int i = 0; i < (*nmols); i++)
    {
        (*deloc)[i] = (double*)malloc(sizeof(double) * (*atomNum)[i]);
        (*totNCM)[i] = 0;
        ...
        else if (ncm == 1) // charge center
        {
            for (int j = 0; j < (*atomNum)[i]; j++)
            {
                (*deloc)[i][j] = top->atoms.atom[index[grp][atomIndex++]].q;
                (*totNCM)[i] += (*deloc)[i][j];
            }
        }
        else if (ncm == 2) // mass center
        {
            for (int j = 0; j < (*atomNum)[i]; j++)
            {
                (*deloc)[i][j] = top->atoms.atom[index[grp][atomIndex++]].m;
                (*totNCM)[i] += (*deloc)[i][j];
            }
        }
        else
        ...
        printf("\nThere are %d molecules in your selection %f\n", *nmols, (*totNCM)[i]);
    }
}
