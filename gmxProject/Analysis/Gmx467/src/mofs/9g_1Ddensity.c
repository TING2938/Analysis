...
#define NFILE asize(fnm)
   ...
   */
    int halfIndex,DIMN,meZ;
	double dbin,dx,*numbin,**posc1,dV;
    printf("CreateMatrix.\n");
  ...
  do{
	...
	for(i = 0; i < nm1; i++)
	{
		if (posc1[i][dim] >= lowPos && posc1[i][dim] < upPos)
		{
  ...
  dV = dbin*Lbox[0]*Lbox[1];
  sprintf(name0, "1D_Density_nbin_%d_mol_%d.dat",nbin,tN1+1);	
  printf("The output name is: %s! for m.s.d based on molecule center!!\n",name0); 
  outputfileR = fopen(name0,"w"); 
  for(i=0;i<nbin;i++)
  {
	  fprintf(outputfileR,"%.4g %.4g\n ", dbin*(i+0.5), numbin[i]/index/dV);
