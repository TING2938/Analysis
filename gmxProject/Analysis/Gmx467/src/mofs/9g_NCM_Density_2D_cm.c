...
*/
  ...
  nbinY=(upy-lowy)/dy;
  nbinZ=(upz-lowz)/dz;
  double **NCMDens, lowPos, upPos, lowD1, lowD2, upD1, upD2, dbin1, dbin2, dV, tmpPos1, tmpPos2 ;
  int  nbin1, nbin2, me1, me2;
  if (DIMN == 0)
  {
	  nbin1 = nbinY; nbin2 = nbinZ;
	  lowPos = lowx; upPos = upx;
	  lowD1 = lowy;  upD1  = upy; 
	  lowD2 = lowz;  upD2  = upz; 
	  dbin1 = dy;    dbin2 = dz;
	  dV    = dy*dz*(upx - lowx);
	  NCMDens = CreateMatrix(nbin1,nbin2);
	  	  
  }
  if (DIMN == 1)
  {
	  nbin1 = nbinX; nbin2 = nbinZ;
	  lowPos = lowy; upPos = upy;
	  lowD1 = lowx;  upD1  = upx; 
	  lowD2 = lowz;  upD2  = upz; 	  
	  dbin1 = dx;    dbin2 = dz;
	  dV    = dx*dz*(upy - lowy);
  ...
  if (DIMN == 2)
  {
	  nbin1 = nbinX; nbin2 = nbinY;
	  lowPos = lowz; upPos = upz;
	  lowD1 = lowx;  upD1  = upx; 
	  lowD2 = lowy;  upD2  = upy;
	  dbin1 = dx;    dbin2 = dy; 
	  dV    = dx*dy*(upz - lowz);	  
...
if (n_ncm == 0)		//added 20190625
{
	do
	{
		...
		for (i = 0; i < nm1; i++)
		{
			...
			{
				if (tmpPos1 >= lowD1 && tmpPos1 <= upD1 && tmpPos2 >= lowD2 && tmpPos2 <= upD2 )
				{
					me1 = (tmpPos1 - lowD1)/dbin1;
					me2 = (tmpPos2 - lowD2)/dbin2;
					NCMDens[me1][me2]++;
				}								
			}
		}
...
if (n_ncm == 1 || n_ncm == 2)		//added 20190625
{
			...
			for (j = 0; j < atoms1; j++)
			{
				...
				if (pos1[i][j][DIMN] >= lowPos && pos1[i][j][DIMN] <= upPos)
				{
					if (tmpPos1 >= lowD1 && tmpPos1 <= upD1 && tmpPos2 >= lowD2 && tmpPos2 <= upD2 )
					{
						me1 = (tmpPos1 - lowD1)/dbin1;
						me2 = (tmpPos2 - lowD2)/dbin2;
						NCMDens[me1][me2] = NCMDens[me1][me2] + atNCM[n_ncm][tN1][j];
					}										
				}				
			}
  ...
  sprintf(name0, "ncm_%d_Density_2D_%s_bin1-%d_bin2-%d_%1.2f-%1.2fnm-dim-%d.dat",n_ncm,MolType,nbin1,nbin2,lowPos,upPos,DIMN);
  outputfileR = fopen(name0,"w");
  for(i=0;i<nbin1;i++)
  {
	  for(j=0;j<nbin2;j++)
	  {
		  fprintf(outputfileR,"%10.3f %10.3f %10.3f\n",(i+0.5)*dbin1+lowD1,(j+0.5)*dbin2+lowD2,NCMDens[i][j]/dV/index);
	  }
  }
